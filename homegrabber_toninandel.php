...
echo "<br>************* GRABBER TONINADEL  ************* START"; 

 
 
$form_query_domain="https://www.i-tonidandel.it"; 
$full_articles_link_n="https://www.i-tonidandel.it/it/cerca?agency_code=&sch_contratto=vendita&price_min=&price_max=&order_by=insert_ts_desc&ricerca_testuale=&page=";
 
 

for($current_page=1;$current_page<24;$current_page++){
 ...
 // echo $sUrlHtml;exit;
    ...
    {$in++;
		...
		//Agenzia  
		$article["agenzia"]= "TONIDANDEL";
        
        //Description
        $article["description"]        = @$article_xpath->query('//div[@class="description-wrapper"]')->item(0)->textContent; 
		...
		if(!is_null( $article_tables )){ 
		  foreach($article_tables as $article_table)
		  {
			...
			foreach($article_table->getElementsByTagName('tr') as $tr)
			{
                ...
                else if(util_string_contains($art_tab_key,"ivell")){
					$art_tab_key="piano";
				}	 				
...
echo "<br>************* GRABBER TONINADEL************* STOP"; 
