...
echo "<br>************* GRABBER BENEDIKTER ************* START"; 


$form_query_domain      ="https://www.benedikter.it";
   
$full_articles_link_b   ="https://www.benedikter.it/wohnimmobilien-kaufen.xhtml?f[40519-19]=kauf&f[40519-23]=0&f[40519-25]=0&f[40519-21]=wohnung&p[obj0]=";
...
for($current_page=1;$current_page<3;$current_page++){
   ...
   // echo $sUrlHtml;exit;
    ...
    $articles  = $xpath->query('//div[@class="list-object"]');
    $article_ind=1; $ind=0;
    ...
    {$ind++;
		...
		//Agenzia  
		$article["agenzia"]= "BENEDIKTER";
         
		//Title
		$article["title"]         = @trim($article_xpath->query('//title')->item(0)->textContent);
		
		//Description
		$article["description"] = @trim($article_xpath->query('//div[@class="panel-body"]')->item(0)->textContent);
	 
		 
		 //localita
		$article["localita"] = @trim($article_xpath->query('//div[@class="subtitle"]')->item(0)->textContent);
	 
 		 
		 //Via
		$article["via"] = @trim($article_xpath->query('//div[@class="subtitle"]')->item(1)->textContent);
		 
		 //Various info
		$article_tables= $article_xpath->query('//div[@class="details-mobile"]/table');  
		if(!is_null( $article_tables )){ 
		  foreach($article_tables as $article_table)
		  {
			...
			foreach($article_table->getElementsByTagName('tr') as $tr)
			{
                ...
                //SAnitazion
				if(util_string_contains($art_tab_key,"emein")){
					$art_tab_key="localita";
				}	
				else if(util_string_contains($art_tab_key,"rtstei")){
					$art_tab_key="via";
				}	
                else if(util_string_contains($art_tab_key,"erkaufsfl")){
					$art_tab_key="superfice";
				}
                else if(util_string_contains($art_tab_key,"aufpre")){
					$art_tab_key="price";
				}
                ...
                else if(util_string_contains($art_tab_key,"mobilien N")){
					$art_tab_key="codice";
				}	
                else if(util_string_contains($art_tab_key,"tock")){
					$art_tab_key="piano";
				}	 				
        ...
        $newarticle["ag_images"]=array(); 
         ...
         //Images 
        $article_table= $article_xpath->query('//img[@class="fotorama-opener"]'); 
