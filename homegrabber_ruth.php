...
echo "<br>************* GRABBER RUTH ************* START"; 


$form_query_domain      ="https://www.ruth-immobilien.com";
  
$full_articles_link_b   ="https://www.ruth-immobilien.com/it/immobili?vermarktungsart=KAUF&realestateType=wohnen&city=1062&order_by=&order_direction=&price=&areas=&rooms=&district=&isAjax=1&page=";
...
for($current_page=1;$current_page<9;$current_page++){
    ...
    $articles  = $xpath->query('//div[@class="asProjectEl"]');
    $article_ind=1; $ind=0;
    ...
    {$ind++;
		...
		//Agenzia  
		$article["agenzia"]= "RUTH IMMOBILIARE";
		...
		$article_tables= $article_xpath->query("//div[contains(@class, 'asTableCell')]");  
		if(!is_null( $article_tables )){ 
		  foreach($article_tables as $article_table)
		  {
		 
				$art_tab_key = @trim(@$article_table->getElementsByTagName('div')->item(1)->nodeValue);  
				$art_tab_val = @trim(@$article_table->getElementsByTagName('div')->item(0)->nodeValue); 

                //SAnitazion
				 if(util_string_contains($art_tab_key,"ocali")){
					$art_tab_key="locali";
				}	
                else if(util_string_contains($art_tab_key,"ommerci")){
        ...
        $newarticle["ag_images"]=array(); 
         ...
         //Images 
        $article_table= $article_xpath->query('//div[@class="item "]'); 
